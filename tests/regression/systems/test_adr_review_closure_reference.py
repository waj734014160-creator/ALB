"""Exact numerical guard for the post-0.3 ADR review closure."""
...
REFERENCE_JSON = ROOT / "refs" / "adr_review_closure_reference_v1.json"
REFERENCE_NPZ = ROOT / "refs" / "adr_review_closure_reference_v1.npz"


def _sha256_array(value: np.ndarray) -> str:
    """Return the byte digest of one contiguous reference array."""

    return hashlib.sha256(np.ascontiguousarray(value).tobytes()).hexdigest()


def test_adr_review_closure_preserves_runtime_numerics_exactly() -> None:
    """Keep ALB, unit-adapter, and real ROSS coupling arrays unchanged."""
