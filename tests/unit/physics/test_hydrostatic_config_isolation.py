"""Regression tests for HydrostaticBearing configuration ownership."""
...
def test_default_hydrostatic_bearings_do_not_share_configuration_state() -> None:
    first = HydrostaticBearing()
    second = HydrostaticBearing()
...
def test_reusing_original_config_after_enabling_vibration_recomputes_velocity() -> None:
    ...
    steady = HydrostaticBearing(config)
    ...
    config.vib = True
    vibrating = HydrostaticBearing(config)
