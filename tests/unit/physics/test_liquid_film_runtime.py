"""Topology-driven liquid-film runtime construction tests."""
...
def test_liquid_film_builder_returns_ready_hydrodynamic_runtime():
    runtime = _build_liquid_film_runtime(_dimensional_config())

    assert isinstance(runtime, _DimensionalMixedFilmRuntime)
...
def test_constructor_supplied_orifices_enable_coupled_calculation(
    ...
    runtime = _build_liquid_film_runtime(config, orifices=orifices)
...
def test_liquid_film_builder_dispatches_nondimensional_config_without_mode_flag():
    runtime = _build_liquid_film_runtime(_nondimensional_config(), x0=10.0)

    assert "mode" not in inspect.signature(_build_liquid_film_runtime).parameters
    assert isinstance(runtime, _NondimensionalMixedFilmRuntime)
    assert isinstance(runtime, BearingRuntimeProtocol)
    assert runtime.lifecycle_state is LifecycleState.READY
...
def test_liquid_film_orifice_requires_one_positive_flow_definition(kwargs):
