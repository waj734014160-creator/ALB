...
本文件保存 coding agent 在本仓库工作时需要遵守的稳定项目级指令。（与 `AGENTS.md` 同源，除「Agent 调度」一节因子 agent 编排工具相关而有意分叉外，其余内容应保持一致。）
...
- 简单任务不启用子 agent：小型修复、单文件审查、简单测试排错优先由主 agent 直接完成。
- 按任务类型选择 agent 类型：只读状态摘要、广撒网检索用 `Explore`；多步研究、复杂代码搜索、需要改文件的多步任务用 `general-purpose`；实现方案设计用 `Plan`。需要并行审计时默认 1-2 个子 agent，再由主 agent 复核和集成。
- 子 agent 输出只作为压缩后的导航层，关键事实在执行前由主 agent 抽查验证。
- 启用子 agent 时按任务复杂度选择模型：轻量、边界清晰、可快速验证的代码检查或小修复用 Haiku；中等复杂度的多文件审计、局部重构、常规训练/远程流程诊断用 Sonnet；高风险或高复杂度任务，如核心训练框架重构、远程长任务控制面修改、数值/物理一致性审计、跨项目集成和最终仲裁，用 Opus。
