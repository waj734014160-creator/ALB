...
+c_{q2}q_j-(p_{sv}-p_{h,j})=0.
$$

固定孔位的压力不是必须等于某个代表节点。令 $W$ 为孔到压力自由度的投影矩阵，
则统一写为
...
`nearest_node` 的每行只有一个单位权重；`element_shape` 使用孔所在单元的原生
形函数。压力-流量切线和阀芯右端因此都通过同一个 $W$ 回投到全局压力自由度，
不能再以代表节点编号代替多节点投影。孔位在本线性化中固定，所以没有
$\hat W$ 项。
