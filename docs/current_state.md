...
- 当前阶段：第五轮发现的 2 个 P1 和 2 个 P2 已在工作树修复；全量工作树运行得到 412 passed、13 skipped、27 subtests passed，唯一失败是新增 11 个节点后旧测试映射的 collection SHA 摘要过期；聚焦门禁为 54 passed、22 subtests passed。尚未提交、未更新 242 节点映射，也未从干净候选提交生成第五轮全量验收，因此合并状态仍保持 `Request changes`。第四轮干净证据仍有效但不能替代本轮待生成证据。
...
16. 第五轮工作树修正已实现：harmonic 重初始化失败后整体失效并封锁状态出口；正式验收拒绝敏感 untracked/ignored 输入并验证 HEAD 前后一致；带标签的嵌套控制器配置严格校验类型和字段；`ALBSV`、`NodimALB`、`NodimALBSV` 不再共享可变默认配置。上述结论待提交后全量验收绑定。
...
| 五轮工作树修正（非发布绑定证据） | 412 passed、13 skipped、27 subtests passed；唯一失败为待提交后重建的测试映射 collection SHA；聚焦 54 passed、22 subtests passed；27 个 v5 数组逐元素精确相等；contracts/core 的 19 个文件 mypy 无问题 | `refs/fifth_review_release_reference_v5.json`、`tests/regression/test_fifth_review_release_reference.py` |
...
- 第五轮修正目前仅有工作树目标测试和 v5 精确参考证据。新的验收门禁会拒绝 `ALB/`、`tests/` 中未提交文件、`tools/` 中未提交 Python 输入和根目录 Python/config 输入（包括被 ignore 的文件），并要求测试前后 HEAD 不变；必须在全部代码和测试提交后运行，才能解除本轮 `Request changes`。
