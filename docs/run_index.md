...
## 模型名称
模型名称需要加入数字前缀，例如 `M0001`，以区分它们与 run ID, 分配规则为查看模型目录中是否已经存在编号，若存在则分配下一个编号。模型名称还需要尽量简短，最主要是包含核心信息，以及区别特征，例如
本次模型修改了损失权重和交叉项权重，那么模型名称可以包含 `fdw200_crossw2` 等关键信息。
