...
| Run and current-status rules | `docs/run_index.md`, `../SURROGATE_TRAIN/docs/current_runtime_status.md` | Keep reusable run-number/path policy in `docs/run_index.md`; keep active run locator state in the owning project's current-status document. | Do not parse Markdown as a database; do not backfill historical runs unless they are reviewed, reused, or archived. |
