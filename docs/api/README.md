...
- 事实来源：`ALB.__all__`、`ALB/api/_config_fields.py`、两类文档生成器、
  `docs/api/public_api_docs.json` 和生成后的公开参考。
...
E:/Anaconda2023/envs/ALB/python.exe -m pytest tests/unit/api/test_public_api_reference.py tests/unit/api/test_bearing_config_reference.py tests/integration/api/test_documented_examples.py -q
```

前两条命令分别重建根 API 与配置字段参考；两条 `--check` 命令只比较内容且不写
文件；pytest 同时固定生成内容、元数据约束和可复制示例。

## 接口变化时的维护顺序

1. 修改并验证公开源码或 `_config_fields.py` 字段元数据。
2. 根 API 语义变化时，在 `public_api_docs.json` 中调整中文参数、异常和示例引用。
3. 运行两类生成器，分别重建 `public_api_reference.md` 和
   `bearing_config_reference.md`。
4. 运行两条 `--check`、目标 pytest 和示例 smoke。

不要手工修改两份生成参考；下一次生成会覆盖手工内容。
