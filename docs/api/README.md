...
- 事实来源：`ALB.__all__`、高级 namespace 的显式导出、`ALB/api/_config_fields.py`、
  四类文档生成器、`docs/api/public_api_docs.json`、
  `docs/api/namespace_reference_docs.json`、`docs/api/config_reference_docs.json` 和生成后的公开参考。
...
- [转子-轴承仿真配置](https://waj734014160-creator.github.io/ALB/api/simulation_config_reference/)
...
[轴承配置字段参考](bearing_config_reference.md)逐项说明所有 bearing family 的文档 envelope、`film`、restrictor、tank、valve、control、transform、thermal、多瓦和 surrogate 字段。[转子-轴承仿真配置参考](simulation_config_reference.md)覆盖 simulation envelope、ROSS Excel 转子、时间网格、mount、三种载荷和 history policy。两页都从 `ALB/api/_config_fields.py` 生成值类型、单位、必填性/默认值、choices、约束、适用范围、原生映射和英文源码合同。

可独立加载和构建的完整 JSON5 位于 [`examples/`](examples/)；依赖 ALBNN package 或 ROSS Excel 的模板位于 [`examples/templates/`](examples/templates/)，只进行结构与占位资源检查，不作为可运行结果。推荐阅读顺序是：先在 IDE/docstring 中确认入口和字段形状，再复制最接近的示例，最后用对应配置参考查询全部约束。
...
通过 Python introspection 读取公开对象、真实签名、公开属性/方法、返回标注、源码
位置和英文 docstring。中文功能说明、参数语义、异常条件和示例保存在
`public_api_docs.json`，不与生成正文混写。

配置参考生成器读取 `ALB/api/_config_fields.py` 中的结构化字段元数据，并分别把稳定的
bearing 与 simulation 字段表和中文 narrative 组合为两页配置参考。公开字段名、原生映射、单位、
适用 family/unit system、缺省值和约束不应在生成器或手册中另建一份事实源。

高级 namespace 生成器以静态方式读取各 namespace 的字面量 `_EXPORTS`，不导入可选
backend；同时从定义源码提取签名、公开成员、源码位置和英文 docstring。中文用途、参数、
返回值、异常、示例、安装 extra 和稳定性标签来自 `namespace_reference_docs.json`。生成结果只包含
显式导出，并分别写入 `docs/api/namespaces/`。
...
E:/Anaconda2023/envs/ALB/python.exe tools/docs/generate_simulation_config_reference.py
...
E:/Anaconda2023/envs/ALB/python.exe tools/docs/generate_simulation_config_reference.py --check
E:/Anaconda2023/envs/ALB/python.exe tools/docs/generate_namespace_reference.py --check
E:/Anaconda2023/envs/ALB/python.exe -m pytest tests/unit/docs tests/unit/api/test_public_api_reference.py tests/unit/api/test_bearing_config_reference.py tests/unit/api/test_simulation_config_reference.py tests/integration/api/test_documented_examples.py tests/integration/docs -q
E:/Anaconda2023/envs/ALB/python.exe -m mkdocs build --strict
```

前四条命令分别重建根 API、轴承配置、仿真配置和高级 namespace 参考；四条 `--check`
命令只比较内容且不写文件；pytest 固定生成内容、源码对应关系、元数据约束、公共内容边界和可复制
...
2. 根 API 语义变化时，在 `public_api_docs.json` 中调整中文参数、异常和示例引用；
   高级 namespace 变化时同步修改 `namespace_reference_docs.json`；配置分组标题或中文摘要变化时修改 `config_reference_docs.json`。
3. 运行四类生成器，重建根 API、轴承配置、仿真配置和高级 namespace 参考。
4. 运行四条 `--check`、目标 pytest、示例 smoke 和 `mkdocs build --strict`。
