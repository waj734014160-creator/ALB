...
`ALB.__all__` 中的 27 个符号是 0.4.5 普通用户入口，覆盖版本元数据、配置、构建、计算、分析、仿真、结果和稳定异常。`ALB.__version__` 表示安装包版本，`ALB.SCHEMA_VERSION` 表示严格 JSON5 配置契约版本。首先查阅[根公开 API](../../api/public_api_reference.md)。
