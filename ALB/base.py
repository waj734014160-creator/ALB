...
class BaseSimpleModel(ABC):
    """
    Simple model with a relatively straightforward calculation process.
    The simple model understands the interfaces of the main model and can call and modify them.
    Therefore, the simple model has significant permissions, but this is set up because the simple model is strongly coupled with the main model.
    """
    ...
    def input(self, *args, **kwargs):
        """
        Set input parameters and obtain necessary information for calculation.
        This can be done directly from the main model or through other methods.
        """
        pass
    ...
    def output(self, *args, **kwargs):
        """
        Calculate and obtain the model's output results.
        """
        pass
...
class BaseSystem:
    ...
    def __init__(self, main_model=None, *simple_models, **args):
        self.args = args
        self.main_model = main_model
        self.simple_models = list(simple_models)
        self.signal = Signal(sys=self)
        self.signal.children = [main_model.signal] + [sm.signal for sm in self.simple_models]
    ...
    def add_simple_model(self, simple_model):
        ...
        if isinstance(simple_model, (list, tuple, np.ndarray)):
            for model in simple_model:
                ...
                self.signal.children.append(model.signal)
                model.signal.father = self.signal
        else:
            self.simple_models.append(simple_model)
            self.signal.children.append(simple_model.signal)
            simple_model.signal.father = self.signal
...
class TimeIter:
    ...
    def __init__(self, start, end, num, **kwargs):
        """
        Abstract method to be implemented by subclasses.
        """
        self.start = start
...
class TimeIterDt:
    ...
    def __init__(self, dt, num, **kwargs):
        """
        Abstract method to be implemented by subclasses.
        """
        self._dt = dt
...
class Signal:
    ...
    def __init__(self, default=False, sys=None, father=None, children: list = None):
        self.father = father
        if children is None:
            children = []
    ...
    def lead(self, signal=None, attr=None):
        """
        transmit signal to children, and if father is not None, transmit father signal to self
        """
        ...
        if self.signal:
            if attr is not None:
                if hasattr(self.sys, attr):
                    ...
                    raise Exception('attr not in sys')
        for child in self.children:
            child.lead(self.signal, attr)
        return self.signal

    def lead_loop(self, attr):
        self.lead(True, attr)
        self.lead(False)
