...
status is delegated to :mod:`ALB.remote.monitor`.
"""
...
def build_runner_content(spec: JobSpec) -> str:
    ...
    env_block = "\n".join(env_lines)
    return f"""# Generated by ALB.remote.job. Edit the JSON config, not this runner.
...
def build_parser() -> argparse.ArgumentParser:
    """Build the CLI parser for ``python -m ALB.remote.job``."""
